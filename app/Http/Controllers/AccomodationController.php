...
{
    private $table="tbl_accn";
    private $module = "accomodation";
    private $view_folder = "accomodation";
    ...
    public function index()
    {
        ...
        if(isset($_GET['bde'])){
            $data['module_data'] = DB::table($this->table)->where('flag','1')->where('Bde',$_GET['bde'])->get();
            $data['title'] = "List of ".$this->module." from Bde ".$_GET['bde'];
        }

        if(isset($_GET['bde']) && isset($_GET['unit'])){
            $data['module_data'] = DB::table($this->table)->where('flag','1')->where('Bde',$_GET['bde'])->where('Unit',$_GET['unit'])->get();
    ...
    public function get(){
        if(Auth::user()->role == "admin")
            return DB::table($this->table)->orderBy('AccnID', 'desc')->where('flag','1')->get();

        if(Auth::user()->role == "user"){
            if(Auth::user()->unit != null || Auth::user()->unit != "")
                return DB::table($this->table)->orderBy('AccnID', 'desc')->where('flag','1')->where('Bde',Auth::user()->bde)->where('Unit',Auth::user()->unit)->get();
            if(Auth::user()->unit == null || Auth::user()->unit == "")
                return DB::table($this->table)->orderBy('AccnID', 'desc')->where('flag','1')->where('Bde',Auth::user()->bde)->get();
        }
    }
    ...
    public function create_edit($id = -1)
    {
        ...
        if ($id !== "-1") {
            $data['data'] = DB::table($this->table)->where('AccnID', $id)->get();
        }
...
//        print_r($data);
        try {
            ...
            if ($id == -1) {
                ...
                DB::table($this->table)->where('AccnID', $id)->update($data);
                DB::table('tbl_logs')->insert($log);
            }
    ...
    {
        DB::table($this->table)->where('AccnID', $id)->update(array('flag' => 0));
        // Create Log
        $log = [
            'LogUser' => Auth::user()->email,
            'Module' => "Accn",
